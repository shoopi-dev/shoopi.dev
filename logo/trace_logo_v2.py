...
# interior shading: #6A0004 vs #8D0104 is a soft gradient, so split a normalised-blur
# field of the red channel instead of hard-classifying each pixel
interior=mouth&(lab==DRK)
sig=1.5*S
R=P[:,:,0]*interior
f=ndimage.gaussian_filter(R,sig)/np.maximum(ndimage.gaussian_filter(interior.astype(float),sig),1e-6)
m_shd = blob(interior&(f>(0x6A+0x8D)/2),"mouth_shade")
m_tng = blob(mouth&(lab==TNG),"mouth_tongue")
m_wht = blob(mouth&(lab==WHT),"mouth_white")

open("logo-v2.svg","w").write(f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 225 225" width="225" height="225" role="img" aria-label="Smiling face logo">
